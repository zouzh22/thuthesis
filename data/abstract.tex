...
\begin{abstract}
  ...
  本文研究意图决策不对齐问题，并基于 OpenClaw 与 AgentWard 设计实现运行时防御机制。该机制由用户真实意图分析、意图对齐检测和动态结果处理三部分组成：先抽取任务目标、约束、敏感对象和风险等级，形成结构化授权边界；再在工具执行前比对智能体决策和工具参数是否仍符合该边界；最后根据偏移程度输出放行、审查或阻断。系统以插件形式接入 OpenClaw，并通过时序同步、状态缓存、规则过滤和异常回退提升真实运行中的稳定性。
